...
void warm_up(int seq[], int N_) {
    ...
    auto build_block_SparseTable = [&]() {
        for(int k = 0;k < M;k++) {
            SparseTable[k][0] = k * S + CarTree_min_idx_map[encoded_tree[k]][0][S - 1];
        }
        for(int j = 1;j <= lg(M);j++) {
            for(int k = 0;k < M;k++) {
                if(k + (1 << (j - 1)) >= M) {
                    ...
                    SparseTable[k][j] = vec[SparseTable[k][j - 1]] < vec[SparseTable[k + (1 << (j - 1))][j - 1]] ? SparseTable[k][j - 1] : SparseTable[k + (1 << (j - 1))][j - 1];
                }
            }
        }
    };
...
int query(int l, int r) {
    ...
    if(L + 1 < R) {
        int j = lg(R - L - 1);
        int idx = vec[SparseTable[L + 1][j]] < vec[SparseTable[R - (1 << j)][j]] ? SparseTable[L + 1][j] : SparseTable[R - (1 << j)][j];
        ans = min(ans, {vec[idx], idx});
    }
...
//    while(Q--) {
//        int l = gen() % N, r = gen() % N;
//        if(vec[query(l, r)] != vec[naive_query(l, r)]) {
//            wrong_q.emplace_back(l, r);
//        }
//    }
//
//    cout << "Wrong queries: " << wrong_q.size() << endl;
//    for(auto &p: wrong_q) {
//        cout << p.first << ' ' << p.second << ' ' << naive_query(p.first, p.second) << endl;
